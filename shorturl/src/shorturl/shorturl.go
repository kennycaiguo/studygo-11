...
func DefaultHandler(w http.ResponseWriter, r *http.Request){
	...
	ip := ""
	if userIp != "" {
		splitIndex := strings.Index(userIp, ":")
		ip = userIp[0:splitIndex]
	}
	if strings.Index(ip, "192.168") != 0 {
		if strings.Index(ip, "58.222.227.142") != 0 {
			if strings.Index(ip, "58.222.223.18") != 0 {
				if strings.Index(ip, "221.230.139.74") != 0 {
					if strings.Index(ip, "127.0.0.1") != 0	{
